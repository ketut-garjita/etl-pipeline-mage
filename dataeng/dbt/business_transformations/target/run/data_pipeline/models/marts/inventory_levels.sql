  ...
  create view "warehouse"."main"."inventory_levels__dbt_tmp" as (
    WITH month_names AS (
...
total_inventory_value AS (
    SELECT
        EXTRACT(YEAR FROM strptime(order_date, '%m/%d/%Y %H:%M')) AS year,
        EXTRACT(MONTH FROM strptime(order_date, '%m/%d/%Y %H:%M')) AS month,
        ROUND(SUM(order_item_product_price * order_item_quantity), 2) AS total_inventory_value
    FROM
        "warehouse"."main"."dim_order"
    GROUP BY
        year, month
),
...
inventory_aging AS (
    SELECT 
        ...
        EXTRACT(MONTH FROM strptime(Order_Date, '%m/%d/%Y %H:%M')) AS month,
        CASE
            WHEN date_diff('day', strptime(Order_Date, '%m/%d/%Y %H:%M'), date '2018-01-01') <= 30 THEN '0-30 days'
            WHEN date_diff('day', strptime(Order_Date, '%m/%d/%Y %H:%M'), date '2018-01-01') <= 60 THEN '31-60 days'
            WHEN date_diff('day', strptime(Order_Date, '%m/%d/%Y %H:%M'), date '2018-01-01') <= 90 THEN '61-90 days'
            ELSE 'Over 90 days'
        END AS age_range,
